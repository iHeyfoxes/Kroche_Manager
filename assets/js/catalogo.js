...
async function catalogoFinalizar(ev){
 ...
 let r;try{r=await supabaseClient.rpc('finalizar_pedido_catalogo',{p_slug:catalogoSlug,p_cliente:cliente,p_telefone:telefone,p_itens:catalogoCarrinho})}catch(error){btn.disabled=false;btn.textContent='Confirmar pedido e enviar no WhatsApp';catalogoToast(error?.message||'Não foi possível concluir o pedido.',true);return}
