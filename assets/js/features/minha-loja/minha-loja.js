...
async function initMinhaLoja(edit=false){
  ...
  if(edit){
    ...
    f.onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target);let banner=p.catalogo_banner;if(fd.get('banner')?.size)banner=await upload('banners',fd.get('banner'),u.id);const data={catalogo_nome:fd.get('catalogo_nome'),catalogo_slogan:fd.get('catalogo_slogan'),whatsapp:fd.get('whatsapp'),catalogo_cor:fd.get('catalogo_cor'),catalogo_cor_botao:fd.get('catalogo_cor_botao'),catalogo_cor_fundo:fd.get('catalogo_cor_fundo'),mostrar_preco:fd.has('mostrar_preco'),mostrar_estoque:fd.has('mostrar_estoque'),mostrar_tempo:fd.has('mostrar_tempo'),catalogo_banner:banner};const r=await supabaseClient.from('usuarios').update(data).eq('id',u.id);if(r.error)msg(r.error.message,false);else{msg('Aparência atualizada!');setTimeout(()=>location.href='minha-loja.html',500)}};
    return;
  }
