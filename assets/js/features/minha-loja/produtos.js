  ...
  async function load(){
    ...
    root.innerHTML=`<div class="store-form-card"><div class="panel-heading"><div><span class="panel-kicker">CATÁLOGO</span><h3>${editId?'Editar produto':'Novo produto'}</h3></div><span class="pill">Minha Loja</span></div>
      <form id="produtoLojaForm" class="grid grid-3">
        ...
        ${formField('Quantidade','quantidade','number','min="0" required')}\n        ${formField('Estoque mínimo','estoque_minimo','number','min="0" required')}
    ...
    if(editing){for(const k of ['nome','preco','quantidade','estoque_minimo','tempo_producao','categoria_id','descricao'])if(f.elements[k])f.elements[k].value=editing[k]??'';f.elements.mostrar_catalogo.value=String(editing.mostrar_catalogo!==false)}
    f.onsubmit=async e=>{e.preventDefault();try{const fd=new FormData(e.target);const id=fd.get('id');let foto=editing?.foto||null;if(fd.get('foto')?.size)foto=await upload('produtos',fd.get('foto'),u.id);const payload={nome:String(fd.get('nome')).trim(),descricao:String(fd.get('descricao')||'').trim()||null,preco:Number(fd.get('preco')),quantidade:Number(fd.get('quantidade')),estoque_minimo:Number(fd.get('estoque_minimo')||0),tempo_producao:fd.get('tempo_producao')?Number(fd.get('tempo_producao')):null,categoria_id:fd.get('categoria_id')?Number(fd.get('categoria_id')):null,mostrar_catalogo:fd.get('mostrar_catalogo')==='true',foto};const r=id?await supabaseClient.from('produtos').update(payload).eq('id',id).eq('usuario_id',u.id):await supabaseClient.from('produtos').insert({...payload,usuario_id:u.id});if(r.error)msg(r.error.message,false);else{msg(id?'Produto atualizado!':'Produto cadastrado!');editId=null;await load()}}catch(error){msg(error?.message||'Não foi possível salvar o produto.',false)}};
