...
async function initEditEncomenda(){
  ...
  const statusHtml=catalogo
    ? '<option>Pendente</option><option>Confirmado</option><option>Em produção</option><option>Pronto</option><option>Entregue</option><option>Cancelado</option>'
    : '<option>Pendente</option><option>Em produção</option><option>Pronto</option><option>Entregue</option>';
  document.getElementById('content').innerHTML='<div class="card"><form id="f" class="grid grid-2">'+formField('Cliente','cliente')+formField('Telefone','telefone')+formField('Produto','produto')+formField('Valor','valor','number','step="0.01"')+formField('Sinal','sinal','number','step="0.01"')+formField('Entrega','data_entrega','date')+'<div><label class="label">Status</label><select class="select" name="status">'+statusHtml+'</select></div><button class="btn btn-primary">Salvar</button></form></div>';
