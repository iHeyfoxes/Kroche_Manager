...
async function initEncomendas(){
  ...
  async function load(){
    ...
    document.getElementById('list').innerHTML=(data||[]).map(x=>{
      ...
      return `<tr><td>${x.data_entrega?new Date(x.data_entrega+'T00:00:00').toLocaleDateString('pt-BR'):'-'}</td><td>${esc(x.cliente)}</td><td>${esc(x.produto)}</td><td>${money(x.valor)}</td><td><span class="pill">${esc(x.status)}</span></td><td><span class="pill ${catalogo?'success':''}">${catalogo?'Catálogo':'Manual'}</span></td><td><a class="btn btn-secondary btn-sm" href="editar-encomenda.html?id=${x.id}">Editar</a> ${catalogo&&x.status!=='Cancelado'?'<button class="btn btn-danger btn-sm" data-cancel-pedido="'+x.pedido_catalogo_id+'">Cancelar</button>':'<button class="btn btn-danger btn-sm" data-id="'+x.id+'">Excluir</button>'}</td></tr>`;
    }).join('')||'<tr><td colspan="7" class="muted">Nenhuma encomenda.</td></tr>';
