...
create table public.vendas (
    ...
    cliente text not null, produto text not null, valor numeric not null,
...
create table public.compras (
    id bigint generated always as identity primary key,
    material text not null, fornecedor text not null, quantidade integer not null,
    valor numeric not null, data timestamptz default now(),
    user_id uuid not null references public.usuarios(id) on delete cascade
);

create table public.receitas (
    id bigint generated always as identity primary key,
    nome text not null, autor text, categoria text, nivel text, youtube text, pdf text,
    observacoes text, favorito boolean default false, data timestamptz default now(),
    user_id uuid not null references public.usuarios(id) on delete cascade
);

create table public.encomendas (
    id bigint generated always as identity primary key,
    cliente text not null, telefone text, produto text not null, valor numeric,
    sinal numeric default 0, data_entrega date, status text default 'Pendente',
    observacoes text, data timestamptz default now(),
    user_id uuid not null references public.usuarios(id) on delete cascade
);
...
create table public.produtos (
    ...
    nome text not null, descricao text, foto text, preco numeric not null,
    quantidade integer not null default 0, tempo_producao integer,
    mostrar_catalogo boolean not null default true, data timestamptz default now()
);

create table public.clientes (
    id bigint generated always as identity primary key,
    usuario_id uuid not null references public.usuarios(id) on delete cascade,
    nome text not null, telefone text, email text, observacoes text,
    data timestamptz not null default now()
);
...
create table public.materiais (
    id bigint generated always as identity primary key,
    usuario_id uuid not null references public.usuarios(id) on delete cascade,
    nome text not null, categoria text, unidade text not null default 'unidade',
    quantidade numeric not null default 0, estoque_minimo numeric not null default 0,
    custo_unitario numeric not null default 0, fornecedor text, observacoes text,
    data timestamptz not null default now(),
    constraint materiais_quantidade_nonnegative check (quantidade >= 0),
    constraint materiais_estoque_minimo_nonnegative check (estoque_minimo >= 0),
    constraint materiais_custo_nonnegative check (custo_unitario >= 0)
);

create table public.leads (
    id bigint generated always as identity primary key,
    usuario_id uuid not null references public.usuarios(id) on delete cascade,
    nome_cliente text not null, telefone_cliente text not null,
    itens jsonb not null, valor_total numeric default 0,
    status text default 'Novo', data timestamptz default now()
);
...
for select to anon using (mostrar_catalogo = true);
create policy "clientes: dono" on public.clientes
for all to authenticated using ((select auth.uid()) = usuario_id) with check ((select auth.uid()) = usuario_id);
create policy "materiais: dono" on public.materiais
for all to authenticated using ((select auth.uid()) = usuario_id) with check ((select auth.uid()) = usuario_id);
...
-- Catálogo público: retorna somente os campos explicitamente permitidos.
...
begin
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
        'id', p.id, 'nome', p.nome, 'descricao', p.descricao, 'foto', p.foto,
        'preco', p.preco, 'quantidade', p.quantidade,
        'tempo_producao', p.tempo_producao, 'mostrar_catalogo', p.mostrar_catalogo
    ...
    return jsonb_build_object(
        'loja', jsonb_build_object(
            'id', v_loja.id, 'nome', v_loja.nome, 'slug', v_loja.slug,
            'whatsapp', v_loja.whatsapp, 'catalogo_nome', v_loja.catalogo_nome,
            'catalogo_slogan', v_loja.catalogo_slogan, 'catalogo_banner', v_loja.catalogo_banner,
            'catalogo_cor', v_loja.catalogo_cor, 'catalogo_cor_botao', v_loja.catalogo_cor_botao,
            'catalogo_cor_fundo', v_loja.catalogo_cor_fundo, 'mostrar_preco', v_loja.mostrar_preco,
            'mostrar_estoque', v_loja.mostrar_estoque, 'mostrar_tempo', v_loja.mostrar_tempo
        ),
        'produtos', v_produtos
    );
end;
...
create policy "fotos-perfil: dono pode inserir" on storage.objects for insert to authenticated
with check (bucket_id = 'fotos-perfil' and (storage.foldername(name))[1] = (select auth.uid())::text);
create policy "fotos-perfil: dono pode atualizar" on storage.objects for update to authenticated
...
create policy "produtos: dono pode inserir storage" on storage.objects for insert to authenticated
with check (bucket_id = 'produtos' and (storage.foldername(name))[1] = (select auth.uid())::text);
create policy "produtos: dono pode atualizar storage" on storage.objects for update to authenticated
...
create policy "banners: dono pode inserir storage" on storage.objects for insert to authenticated
with check (bucket_id = 'banners' and (storage.foldername(name))[1] = (select auth.uid())::text);
create policy "banners: dono pode atualizar storage" on storage.objects for update to authenticated
