-- Consolida o fluxo de pedidos do catálogo e garante a integridade do vínculo com vendas.
-- O banco já possui versões anteriores dessas estruturas; este migration é idempotente.
...
alter table public.encomendas
  add column if not exists pedido_catalogo_id bigint references public.pedidos_catalogo(id) on delete set null;
...
create policy "pedidos_catalogo: dono"
...
create policy "pedido_itens: dono"
on public.pedido_itens
for all to authenticated
using (
  exists (
    select 1
    from public.pedidos_catalogo p
...
begin
  if pg_catalog.btrim(coalesce(p_cliente,'')) = '' then
    raise exception 'Informe seu nome.';
  end if;

  if pg_catalog.length(pg_catalog.btrim(p_cliente)) > 120 then
    raise exception 'O nome é muito longo.';
  end if;

  if pg_catalog.btrim(coalesce(p_telefone,'')) = '' then
    raise exception 'Informe seu telefone.';
  end if;

  if pg_catalog.length(pg_catalog.btrim(p_telefone)) > 30 then
    raise exception 'O telefone é muito longo.';
  end if;

  if pg_catalog.jsonb_typeof(p_itens) <> 'array'
     or pg_catalog.jsonb_array_length(p_itens) = 0 then
  ...
  loop
    if v_item.id is null
       ...
       or v_item.quantidade > 1000 then
    ...
    if coalesce(v_produto.quantidade, 0) < v_item.quantidade then
      raise exception 'Estoque insuficiente para: % (disponível: %).',
        v_produto.nome, v_produto.quantidade;
    end if;
  ...
  insert into public.encomendas(
    cliente, telefone, produto, valor, sinal, status,
    observacoes, user_id, pedido_catalogo_id
  )
  values(
    pg_catalog.btrim(p_cliente),
    pg_catalog.btrim(p_telefone),
    v_produtos_resumo,
    v_total,
    0,
    'Pendente',
